...
class ReducedHaplotypeInfo
{
    public:
        ...
        vector<vector<int> > uniqueIntHaps;
...
class findUnique
{
  ...
  double FlushBlocks(vector<ReducedHaplotypeInfo> &HapInfo, vector<variant> &varList, int LastflushPos,vector<String> & haplotypes,vector<int> &MaxAllele, vector<int> & cost,
                   vector<int> & bestComplexity, vector<int> & bestSlice, vector<vector<int> > &bestIndex);
